...
-- =========================================================
-- MAIL TAB — categories / plant names
-- =========================================================
...
-- =========================================================
-- MAIL FRUITS TAB — value calculation
-- =========================================================
...
local RefreshStatusLabel = New("TextLabel", {
    ...
    TextColor3 = Theme.TextDim,
    BackgroundTransparency = 1,
    Position = UDim2.new(0.62, 6, 0, 44),
...
-- Item dropdown popup — parented to Body so it can paint over BottomPanel
...
SendButton.Activated:Connect(function()
    ...
    task.spawn(function()
        ...
        local success = pcall(function()
            ...
            print("=== SENDING BATCH ===")
            for i, item in ipairs(finalBatch) do
                print(i, item.Category, item.ItemKey, item.Count)
            end
            print("=====================")
        ...
        if success then
            ...
            LogMailHistory(user, queue)
            SendMailWebhook(user, queue, "Mail")
            queue = {}
            RefreshQueueDisplay()
            task.wait(0.6)
            RefreshItemOptions()
...
local function RefreshFruitsQueue()
    for _, child in ipairs(FruitsQueueList:GetChildren()) do
        if child:IsA("Frame") then child:Destroy() end
    end

    local totalValue = 0
    for _, entry in ipairs(fruitsQueue) do
        totalValue += entry.Value
    end

    FruitsQueueHeader.Text = string.format("Queue \u{2022} %d items", #fruitsQueue)
    FruitsQueueTotalLabel.Text = "TOTAL VALUE: " .. FormatCompactSheckles(totalValue)
    FruitsQueueList.CanvasSize = UDim2.new(0, 0, 0, #fruitsQueue * 34)

    local emptyLabel = FruitsQueueList:FindFirstChild("QueueEmpty")
    if emptyLabel then
        emptyLabel.Visible = (#fruitsQueue == 0)
    end

    OpenFruitPickerBtn.Text = "Choose fruits (" .. #fruitsQueue .. "/20)"

    for i, entry in ipairs(fruitsQueue) do
        local row = New("Frame", {
            BackgroundColor3 = Theme.Surface2,
            BorderSizePixel = 0,
            Size = UDim2.new(1, 0, 0, 30),
...
FruitsSendButton.Activated:Connect(function()
    ...
    task.spawn(function()
        ...
        local total = #fruitsQueue
        local sent = 0
        local batchSize = 20
        local cooldown = 10
        local historyItems = {}

        while #fruitsQueue > 0 do
            local batch = {}
            local historyBatch = {}
            local currentBatchSize = math.min(batchSize, #fruitsQueue)

            for i = 1, currentBatchSize do
                local fruit = fruitsQueue[i]
            ...
            SendMailWebhook(user, historyBatch, "Mail Fruits")
...
print("[MailScript] Loaded — MAIL + MAIL FRUITS + MAIL HISTORY (combined)")
